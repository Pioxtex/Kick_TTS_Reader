...
function prepForTTS(text, maxLen, profanityFilter, filter) {
  ...
  // odrzuc ściany znaków / powtórzenia
...
export class TTSBot {
  constructor(onLog, onState, onMuted) {
    ...
    // konfig – startuj od globalnych defaults + to co w pliku settings.json
  ...
  // === Pomocnicze ===
  // ASCII, małe litery, bez @ i polskich znaków
  _normName(s) {
    return String(s || '')
      .normalize('NFKD')
      .replace(/[\u0300-\u036f]/g, '')   // diakrytyki
      .replace(/[@\s]+/g, '')            // usuń @ i spacje
      .toLowerCase();
  }

  // Pobiera nazwę użytkownika w sposób odporny na różne struktury Kicka
  _getUsername(msg) {
    return (
      msg?.sender?.username ||
  ...
  // Sprawdza, czy użytkownik jest moderatorem, adminem lub właścicielem kanału
  _isModOrOwner(msg) {
    const s = msg?.sender || {};

    // 1) Booleany spotykane w payloadach Kicka
    if (s.is_broadcaster || s.is_owner || s.is_moderator || s.is_admin || s.is_streamer) return true;

    // 2) Jeśli autor wiadomości to właściciel kanału
    const user = this._getUsername(msg).toLowerCase();
    if (this.channel && user && this.channel.toLowerCase() === user) return true;

    // 3) Tablice ról / badge’y w różnych polach
    const arrays = [
      s.roles, s.role, s.user_roles,
      s.badges, s.identity?.badges
    ].filter(Boolean);

    for (const arr of arrays) {
      for (const it of arr) {
        const v = (typeof it === 'string'
          ? it
          : (it?.type || it?.name || it?.slug || '')).toString().toLowerCase();
        if (!v) continue;
  ...
  async start(channel, options = {}) {
    ...
    // start klienta + reconnect/backoff
    ...
    const connectOnce = async () => {
      ...
      this.client.on('ChatMessage', (msg) => {
        try {
          ...
          // PRIORYTET 1: !tts (komendy mod + tekst)
          if (low.startsWith('!tts')) {
            ...
            if (rest) {
              ...
              const adminSet = new Set(['mute','unmute','toggle','skip','clear','status','rate','volume','maxqueue','voice']);
              if (adminSet.has(sub)) {
                if (!isMod) {
                  const s = msg?.sender || {};
                  const brief = {
                ...
                this._handleCommand(msg, rest); // ważne: CAŁE msg
                return; // komend moderatorskich nie czytamy
              }
            }
          ...
          // PRIORYTET 2: inne !komendy — nie czytamy
          if (isCommand) return;
      ...
      if (typeof this.client.connect === 'function') {
        try { await this.client.connect(); }
          ...
          this.onLog('[ws] connect error: ' + (e?.message || e));
          throw e;
        }
      }
    };
    ...
    switch (sub) {
      ...
      case 'rate': {
        ...
        if (!Number.isFinite(n) || n < 0.5 || n > 2.0) {
          this.onLog('[cmd] rate: podaj 0.5–2.0, np. "!tts rate 1.1"');
          return;
        }
      ...
      case 'volume': {
        ...
        if (!Number.isFinite(n) || n < 0 || n > 100) {
          this.onLog('[cmd] volume: podaj 0–100, np. "!tts volume 85"');
          return;
        }
      ...
      case 'maxqueue': {
        ...
        if (!Number.isFinite(n) || n < 1 || n > 200) {
          this.onLog('[cmd] maxqueue: podaj 1–200, np. "!tts maxqueue 80"');
          return;
        }
      ...
      case 'voice': {
        ...
        if (!name) { this.onLog('[cmd] voice: podaj nazwę głosu'); return; }
        this.setVoice(name);
      ...
      case 'prefix': {
        const val = rest; // może zawierać spacje i {user}
        if (!val) { this.onLog('[cmd] prefix: podaj wartość, np. "!tts prefix {user}: "'); return; }
      ...
      default: {
        // nieznana subkomenda – potraktujemy ją jak treść do przeczytania (jeśli speakTtsCommands=ON)
        if (!this.opts.speakTtsCommands) return;
  ...
  _shouldAccept(userLC, raw) {
    ...
    // czyszczenie starego śmietnika co jakiś czas
  ...
  setOption(key, value) {
    // akceptuj klucze istniejące w bieżących opcjach LUB w defaults
